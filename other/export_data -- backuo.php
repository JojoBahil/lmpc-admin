...
//             $sequenceNumber = '000001';
    ...
    while($row = mysqli_fetch_assoc($result)){
        ...
        //echo '017188||P|N|0|GPRLBP||||||||4|GPRDP01||1|||||||||||||||||||||003||'.$title.'|'.$fname.'|'.$mname.'||'.$lname.'|'.$sex.'|'.$motherfullname.'|'.$marital_status.'|608|'.$newbirthdate.'|'.$birth_place.'|608|2||'.$emboss_name.'||Y|Compliant||||C|'.$current_address.'||||||||'.$contact.'||'.$permanent_address.'||||||||'.$contact.'||||Y|Y|||'.$isd_code.'|'.$contact.'|'.$email.'|en|'.$pickup.'|||||||||||||'.$id_type.'|'.$id_number.'|'.$id_expire_date.'||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||7|||||||5'."\r\n";
        //echo '017188||P|N|0|GPRLBP||||||||4|GPRDP01||1|||||||||||||||||||||003||'.$title.'|'.$fname.'|'.$mname.'||'.$lname.'|'.$sex.'|'.$motherfullname.'|'.$marital_status.'|608|'.$newbirthdate.'|'.$birth_place.'|608|2||'.$emboss_name.'||Y|Compliant||||C|'.$current_address.'||||||||'.$contact.'||'.$permanent_address.'||||||||'.$contact.'||||Y|Y|||'.$isd_code.'|'.$contact.'|'.$email.'|en|'.$pickup.'|'.$servicing_branch.'||||||||||||'.$id_type.'|'.$id_number.'|'.$id_expire_date.'||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||7|||||||5'."\r\n";
        //echo '017188||P|N|0|GPRLBP||||||||4|GPRDP01||0|||||||||||||||||||||003||'.$title.'|'.$fname.'|'.$mname.'||'.$lname.'|'.$sex.'|'.$motherfullname.'|'.$marital_status.'|608|'.$newbirthdate.'|'.$birth_place.'|608|2||'.$emboss_name.'||Y|Compliant||||C|'.$current_address.'||||||608||'.$contact.'||'.$permanent_address.'|'.$permanent_address.'|'.$permanent_address.'||||||'.$contact.'||||Y|Y|||'.$isd_code.'|'.$contact.'|'.$email.'|en|'.$pickup.'|'.$servicing_branch.'||||||||||||'.$id_type.'|'.$id_number.'|'.$id_expire_date.'||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||7|||||||5'."\r\n";
        echo '019372||P|N|0|GPRLBP||||||||4|GPRDP01||0|||||||||||||||||||||003||'.$title.'|'.$fname.'|'.$mname.'||'.$lname.'|'.$sex.'|'.$motherfullname.'|'.$marital_status.'|608|'.$newbirthdate.'|'.$birth_place.'|608|2||'.$emboss_name.'||Y|Compliant||||C|'.$lbp_branch_code.' '.$current_address.'||||||608||'.$contact.'||'.$permanent_address.'|'.$permanent_address.'|'.$permanent_address.'||||||'.$contact.'||||Y|Y|||'.$isd_code.'|'.$contact.'|'.$email.'|en|'.$pickup.'|'.$servicing_branch.'||||||||||||02|'.$id_number.'|'.$id_expire_date.'||||||||||||||||||||'.$hei_name.'|'.$award_no.'|'.$batch_number.'|'.$hei_psg_region.'|||||||||||||||||||||||||||||||||||||7|||||||5|||||||||||||||||||||||||||||||||||'."\r\n";
...
//         $file_count_offset = $file_count_limit - 50;
        ...
        $sqlDownload50 = "SELECT award_no, pdf_attachment FROM vw_for_admin WHERE application_datfile_name ='".$_POST['sel_reexport']."' AND application_datfile_batch = '".$batch_number."'";
...
//EXPORT TEXT FILE ---------------------------------------------------------------------------------------------
    ...
    if(isset($_POST['btn_download_text'])){
        ...
        $sqlDownload50 = "SELECT award_no, pdf_attachment, pickup_at_hei, hei_uii, lbp_branch_code, lbp_branch FROM vw_for_admin WHERE application_datfile_name ='".$batch_name."' AND application_datfile_batch = '".$batch_number."'";
        $resDownload50 = mysqli_query($connect, $sqlDownload50);
        $checkDownload50 = mysqli_num_rows($resDownload50);
