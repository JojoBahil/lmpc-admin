    ...
    while($row = mysqli_fetch_assoc($result)){
        ...
        if($checkCurrentDATFile > 0){
            while($datfile_row = mysqli_fetch_assoc($resCurrentDATFile)){
                ...
                $modified_award_no = str_replace("-", "", str_replace("TES", "", $award_no));
                ...
                echo '019372||P|N|0|GPRLBP||||||||4|GPRDP01||0|||||||||||||||||||||003||'.$title.'|'.$fname.'|'.$mname.'||'.$lname.'|'.$sex.'|'.$motherfullname.'|'.$marital_status.'|608|'.$newbirthdate.'|'.$birth_place.'|608|2||'.$emboss_name.'||Y|Compliant||||C|'.$lbp_branch_code.' '.$current_address.'|||GP001 '.$lbp_branch_code.'|||608||'.$contact.'||'.$permanent_address.'|'.$permanent_address.'|'.$permanent_address.'||||||'.$contact.'||||Y|Y|||'.$isd_code.'|'.$contact.'|'.$email.'|en|'.$pickup.'|'.$servicing_branch.'||||||||||||'.$id_type.'|'.$id_number.'|'.$id_expire_date.'||||||||||||||||||||'.$hei_name.'|'.$award_no.'|'.$application_datfile_batch.'|'.$hei_psg_region.'|||||||||||||||||||||||||||||||||||||7|||||||5|||||||'.$modified_award_no.'||||||||||||||||||||||||||||||||||||||||||||||||'."\r\n";
